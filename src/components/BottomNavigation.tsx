...
export function BottomNavigation({ activeTab, onTabChange, unreadMessages = 0, hasJoinedGames = false, hasCreatedGame = false }: BottomNavigationProps) {
            ...
            >
              ...
              {tab.badge && tab.badge > 0 && (
                <div className="absolute -top-1 -right-1 w-5 h-5 bg-red-600 rounded-full flex items-center justify-center">
                  <span className="text-xs text-white">{tab.badge > 9 ? '9+' : tab.badge}</span>
                </div>
              )}
            </button>
          );
